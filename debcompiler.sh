...
readonly DEVICE_IP="192.168.1.73"
readonly DEVICE_PORT="22"
...
if [[ -d ${TMPFOLDER}/Library/MobileSubstrate/DynamicLibraries ]]; then
  ...
  scp -P "${DEVICE_PORT}" ${TMPFOLDER}/Library/MobileSubstrate/DynamicLibraries/* "root@${DEVICE_IP}":/Library/MobileSubstrate/DynamicLibraries/
fi
...
if [[ -d ${TMPFOLDER}/Library/PreferenceBundles ]]; then
  ...
  scp -P "${DEVICE_PORT}" -pr ${TMPFOLDER}/Library/PreferenceBundles/*/ "root@${DEVICE_IP}":/Library/PreferenceBundles/
fi
...
if [[ -d ${TMPFOLDER}/Library/PreferenceLoader ]]; then
  ...
  scp -P "${DEVICE_PORT}" -pr ${TMPFOLDER}/Library/PreferenceLoader/*/ "root@${DEVICE_IP}":/Library/PreferenceLoader/
fi
